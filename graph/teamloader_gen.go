...
func (l *TeamLoader) Load(key string) (*models.Team, error) {
...
func (l *TeamLoader) LoadThunk(key string) func() (*models.Team, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*models.Team, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *TeamLoader) LoadAll(keys []string) ([]*models.Team, []error) {
	results := make([]func() (*models.Team, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	teams := make([]*models.Team, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TeamLoader) LoadAllThunk(keys []string) func() ([]*models.Team, []error) {
	results := make([]func() (*models.Team, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TeamLoader) Prime(key string, value *models.Team) bool {
	l.mu.Lock()
	var found bool
...
func (l *TeamLoader) unsafeSet(key string, value *models.Team) {
	if l.cache == nil {
		l.cache = map[string]*models.Team{}
	}
	l.cache[key] = value
}
